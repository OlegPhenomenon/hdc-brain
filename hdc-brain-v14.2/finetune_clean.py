"""Finetune v14.1 on dialog data WITHOUT CogitLayer — clean baseline for paper.

Same dataset, same hyperparameters as finetune_dialog.py, only CogitLayer removed.
Goal: a standalone finetuned checkpoint that does not depend on external
associative memory at inference.
"""
...
# Config — identical to finetune_dialog.py (minus CogitLayer)
...
def forward_clean(X, Y):
    """Standard forward — no CogitLayer hint."""
...
log(f"\nStarting CLEAN finetune (no CogitLayer): {MAX_ITERS} iters, LR={LR}")
    ...
    if it > 0 and it % 500 == 0:
        ...
        if best:
            ...
            torch.save({
                ...
                "clean": True,
            }, "best_finetune_clean_v14_1.pt")
        torch.save({
            "model": model.state_dict(), "config": cfg,
            "val_loss": vl, "iter": it, "vocab_size": V, "finetune": True,
            "clean": True,
        }, "last_finetune_clean_v14_1.pt")
        log("=" * 60)
...
torch.save({
    "model": model.state_dict(), "config": cfg,
    "val_loss": best_val, "iter": it, "vocab_size": V, "clean": True,
}, "last_finetune_clean_v14_1.pt")
